...
int main()
{
int i,j=0,n,a[50],frame[10],nof,k,avail,count=0;
...
for(i=1;i<=n;i++){
    ...
    if (avail==0){         /*If page is not present in the frames insert  it*/
		frame[j]=a[i];
        j=(j+1)%nof;        /* this expression helps in maintaining FIFO order*/
        count++;           /* count no of page faults */     
        for(k=0;k<nof;k++)    /* prints frames data as there is alternation occured*/
        printf("%d\t",frame[k]);                                               
}
printf("\n");
}
